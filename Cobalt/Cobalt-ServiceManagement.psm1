...
param (
    ...
    [Parameter(HelpMessage="The name of the OData User entity property used to contain granted OAuth scopes")][string]$ScopeAttribute,
	[Parameter(HelpMessage="The name of the OData User entity property used to indicate that it is enabled")][string]$EnabledAttribute = "userEnabled",
	[Parameter(HelpMessage="The URL path for the identity provider endpoint, e.g. '/csp/identity/'")][string]$Path,
	[Parameter(HelpMessage="The name of the OData User property to use as a logon name")][string]$PrincipalAttribute = 'cobaltUsername',
    ...
    [Parameter(HelpMessage="The supported OAuth scopes in JSON format")]$Scopes = '{ "name" : "openid", "claims" : { "sub" : "EntityUUID" } }',
    ...
    [Parameter(HelpMessage="The attribute used to store the time-based one time password secret")][string]$TOTPAttribute,
	...
	[Parameter(HelpMessage="The zone this identity service is associated with")][string]$Zone,
	[Parameter(HelpMessage="The attribute to use for checking passwords")][string]$PasswordAttribute = 'cobaltPassword',
	[Parameter(HelpMessage="The namespace-qualified name of the entity type to use for authentication")]$EntityType = 'com.viewds.cobalt.User',
	...
	if(-not [String]::IsNullOrEmpty($PasswordAttribute)){
		$properties.Add('PasswordAttribute', $PasswordAttribute)
	}
	if(-not [String]::IsNullOrEmpty($ScopeAttribute)){
		$properties.Add('ScopeAttribute', $ScopeAttribute)
	}
	if(-not [String]::IsNullOrEmpty($EnabledAttribute)){
		$properties.Add('EnabledAttribute', $EnabledAttribute)
	}
	if(-not [String]::IsNullOrEmpty($TOTPAttribute)){
		$properties.Add('TOTPAttribute', $TOTPAttribute)
	}
	if(-not [String]::IsNullOrEmpty($PrincipalAttribute)){
		$properties.Add('PrincipalAttribute', $PrincipalAttribute)
	}
	else {
		# Try to get the principal attribute from one of the OData services for the tenant
		$odataEndpoint = Get-CobaltEntities -Connection $Connection -ODataType 'com.viewds.cobalt.ODataEndpoint' -ODataFilter "Tenant eq '$Tenant'" | Select -First 1
		if($odataEndpoint -eq $null) {
			Throw "Could not obtain PrincipalAttribute from tenant '$Tenant' OData endpoint"
		}
		$properties['PrincipalAttribute'] = $odataEndpoint.PrincipalAttribute
	}
	...
	if($UseDefaultScopes){
		$properties.Add('Scopes', $script:DefaultScopes)
	}
...
, @"
{
    "name" : "phone",
    "claims" : {
        "phone_number" : "com-example-csp-TelephoneNumber",
...
, @"
{
    "name" : "profile",
    "claims" : {
        "name" : "cobaltEntryName",
        "family_name" : "com-example-csp-Surname",
        "given_name" : "com-example-csp-GivenName",
        "middle_name" : null,
        "nickname" : null,
        "preferred_username" : "com-example-csp-Email",
        "profile" : null,
        "picture" : null,
        "website" : null,
        "gender" : null,
